...
contract DeployerCrossChainBridge is Script {
    ...
    function run() public {
        ...
        address chainMultiSign = vm.envAddress("MULTI_SIGNER");
        ...
        TransparentUpgradeableProxy proxyMessageManager =
            new TransparentUpgradeableProxy(address(emptyContract), chainMultiSign, "");
        ...
        messageManagerProxyAdmin.upgradeAndCall(
            ...
            abi.encodeWithSelector(MessageManager.initialize.selector, deployerAddress, poolManager)
        );
        ...
        poolManagerProxyAdmin.upgradeAndCall(
            ...
            abi.encodeWithSelector(
                PoolManager.initialize.selector, deployerAddress, messageManager, relayerAddress, deployerAddress
            )
        );
        ...
        string memory path = string(abi.encodePacked("deployed_addresses_", chainIdStr, ".json"));
        string memory data = string(
            abi.encodePacked(
                '{"proxyMessageManager": "',
                vm.toString(address(proxyMessageManager)),
                '", ',
                '"proxyPoolManager": "',
                vm.toString(address(proxyPoolManager)),
                '"}'
            )
        );
