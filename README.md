...
```solidity
// getReward() 函数缺少精度因子
// 建议：与 WithdrawOrClaimBySimpleAsset 保持一致
uint256 _Reward = (Amount * TotalFee * 1e18) / TotalAmount;
Reward += _Reward / 1e18;
```
...
```solidity
// WithdrawOrClaimBySimpleAsset 中使用 swap-and-pop 删除元素
// 可能导致某些记录被跳过
// 建议：使用标记删除或重新设计删除逻辑
```
...
```solidity
// getPrincipal() 和 getReward() 的三层循环
// 建议：添加分页查询或限制返回数量
```
